...
class EditFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        )[EditProdukViewModel::class.java]

        produk?.let { item ->
            binding.etNamaBarang.setText(item.name)
            binding.etKategoriBarang.setText(item.category)
            binding.etHarga.setText(item.price.toString())
            binding.etStokGudang.setText(item.stockGudang.toString())
            binding.etStokKantin.setText(item.stockKantin.toString())
        }
